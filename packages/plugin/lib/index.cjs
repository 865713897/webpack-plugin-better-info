...
const getCompressSize = (filePath) => {
  return new Promise((resolve, reject) => {
    const _path = path__default.resolve(process.cwd(), filePath);
    fs.readFile(_path, (error, content) => {
      if (error) {
        return reject(error);
      }
      zlib__default.gzip(content, (err, compressedData) => {
        if (err) {
          return reject(err);
        }
...
class Progress {
  ...
  getFormattedString(percent, elapsed, eta, rate, force) {
    return this.fmt.replace(":current", this.current + "").replace(":total", this.total + "").replace(":elapsed", isNaN(elapsed) ? "0.0" : (elapsed / 1e3).toFixed(1) + "").replace(
      ":eta",
      force ? chalk__default.gray("emitting after emit") : isNaN(eta) || !isFinite(eta) ? "0.0s" : (eta / 1e3).toFixed(1) + "s"
...
class Logger {
  ...
  event(message) {
    console.log(`${chalk__default.magenta("event")} - ${message}`);
  }
  ready(message) {
    console.log(`${chalk__default.green("ready")} - ${message}`);
  }
  info(message) {
    console.log(`${chalk__default.cyan("info")}  - ${message}`);
  }
  error(message) {
...
async function displayAssets(assets, outputPath) {
  ...
  newAssets.forEach(({ name, size, zipSize, originalSize }) => {
    const colorZipSize = originalSize > maxFileSize ? chalk__default.red(zipSize) : chalk__default.green(zipSize);
    console.log(
...
class WebpackPluginBetterInfo {
  constructor(options) {
    ...
    this.bar = new Progress(`\u25CF ${chalk__default.whiteBright.bold("web")} :bar (:percent) :eta`, {
      complete: options.complete,
      incomplete: options.incomplete,
      width: options.width,
      total: options.total,
  ...
  apply(compiler) {
    ...
    const processPlugin = new webpack__default.ProgressPlugin((percentage, message, ...args) => {
...
module.exports = WebpackPluginBetterInfo;
